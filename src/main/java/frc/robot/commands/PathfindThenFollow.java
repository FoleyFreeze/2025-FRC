...
public class PathfindThenFollow extends Command {
    RobotContainer r;
    boolean flipRobot;
    ...
    public PathfindThenFollow(RobotContainer r, Supplier<Pose2d> poseSupplier, boolean flipRobot) {
    ...
    public void initialize() {
        ...
        Pose2d offsetTarget = targetPose.plus(finderDelta);

        // convert pose angle to desired drive direction insead of the angle the bot is facing
        Translation2d travelVector =
                targetPose.getTranslation().minus(offsetTarget.getTranslation());
        Pose2d startPose = new Pose2d(offsetTarget.getTranslation(), travelVector.getAngle());
        ...
        c = AutoBuilder.pathfindThenFollowPath(path, pathConstraints);
        c.initialize();
    }
