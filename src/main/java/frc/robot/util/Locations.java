...
public class Locations {
    ...
    public static Transform2d halfRobotAlgaeRight =
            new Transform2d(
                    robotLength / 2.0 + Units.inchesToMeters(2.5),
                    Units.inchesToMeters(-2.5 + 1.2), // less offset from this side
                    Rotation2d.kZero);
    ...
    static double bargeShrinkage = 0.7 + 0.15;
    ...
    public static Pose2d getCageLocation(RobotContainer r) {
        double midX = tags.getFieldLength() / 2;
        ...
        if (isBlue()) {
            switch (r.controlBoard.selectedCage) {
                case LEFT:
                    return new Pose2d(
                            midX - driveDeltaX, quarterY * 3 + cageDeltaY, Rotation2d.kCW_90deg);
                case MIDDLE:
                default:
                    return new Pose2d(midX - driveDeltaX, quarterY * 3, Rotation2d.kCW_90deg);
                case RIGHT:
                    return new Pose2d(
                            midX - driveDeltaX, quarterY * 3 - cageDeltaY, Rotation2d.kCW_90deg);
            }
            ...
            switch (r.controlBoard.selectedCage) {
                case LEFT:
                    return new Pose2d(
                            midX + driveDeltaX, quarterY - cageDeltaY, Rotation2d.kCCW_90deg);
                case MIDDLE:
                default:
                    return new Pose2d(midX + driveDeltaX, quarterY, Rotation2d.kCCW_90deg);
                case RIGHT:
                    return new Pose2d(midX + driveDeltaX, quarterY, Rotation2d.kCCW_90deg);
            }
        }
