...
public class ModuleIOTalonFX implements ModuleIO {
    ...
    public void updateInputs(ModuleIOInputs inputs) {
        ...
        inputs.driveTempF = driveTemp.getValue().in(Fahrenheit);
        ...
        inputs.turnTempF = turnTemp.getValue().in(Fahrenheit);
    ...
    public void setDriveVelocityFF(double velocityRadPerSec, double accel) {
        double velocityRotPerSec = Units.radiansToRotations(velocityRadPerSec);
        double accelRotations = Units.radiansToRotations(accel);
        driveTalon.setControl(
                switch (constants.DriveMotorClosedLoopOutput) {
                    case Voltage -> velocityVoltageRequest
                            .withVelocity(velocityRotPerSec)
                            .withAcceleration(accelRotations);
                    case TorqueCurrentFOC -> velocityTorqueCurrentRequest.withVelocity(
                            velocityRotPerSec);
                });
    }
    ...
    boolean brakeStatus = false;

    @Override
    public void setBrakes(boolean on) {
        if (on != brakeStatus) {
            brakeStatus = on;
            driveTalon.setNeutralMode(on ? NeutralModeValue.Brake : NeutralModeValue.Coast);
        }
    }
}
